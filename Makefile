...
crossbuild: | $(PROMU)
	@echo ">> crossbuilding all binaries"
	$(PROMU) crossbuild -v -p linux/amd64 -p linux/arm64 -p linux/ppc64le -p darwin/amd64 -p darwin/arm64 -p windows/amd64 -p freebsd/amd64 --parallelism=1 --parallelism-thread=0
endif
