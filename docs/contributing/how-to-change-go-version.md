...
2. Edit [.circleci/config.yaml](../../.circleci/config.yml) and edit ` - image: circleci/golang:<go version>` to desired Golang version. This will ensure that all docker images and go tests are using desired Golang version. How to verify? Invoke `docker pull improbable/thanos:<version> --version`
